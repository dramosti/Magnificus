...
{
    public partial class FormMulta : FormPadraoCadastro
    {
        [Inject]
        public IMultasService multasService { get; set; }

        MultasModel multasModel = new MultasModel();
        ...
        {
            multasService.Delete(Convert.ToInt32(txtCodigo.Text));
            base.Excluir();
            if (iRetPesquisa != null)
            {
                base.MoveProximoItem();
                multasModel = multasService.GetMulta((int)iRetPesquisa);
                PopulaForm();
            }
        }
        private void ExcluirTodos()
        {
            ...
            for (int i = 0; i < lParaExcluir.Count; i++)
            {
                try
                {
                    ...
                    multasService.Delete((int)lParaExcluir[i]);
        ...
        public override void Salvar()
        {
            try
            {
                ...
                PopulaTabela();
                multasService.Save(multasModel);
        ...
        public override void Cancelar()
        {
            try
            {
                if (HLPMessageBox.MsgCancelar())
                {
                    ...
                    {
                        multasModel = multasService.GetMulta(Convert.ToInt32(txtCodigo.Text));
                        PopulaForm();
                        HabilitaBotoes(1);
                    }
                    base.Cancelar();
